...
export default async function HalamanRiwayat({
  ...
  return (
    <div className="pb-6">
      {/* ------------------------------------------------------- Kepala */}
      <header className="bg-brand-700 pt-safe px-5 pb-16">
        ...
        <div className="mt-4 flex items-center justify-between rounded-[var(--radius-card)] bg-white/12 px-1.5 py-1.5">
          <Link
            href={geser(-1)}
      ...
      <div className="-mt-12 px-5">
        <div className="bg-surface rounded-[var(--radius-sheet)] p-5 shadow-[var(--shadow-raised)]">
          ...
          <p className="tnum text-body mt-1 text-[32px] leading-none font-extrabold">
            {ringkas.hadir} <span className="text-muted text-base font-bold">hari</span>
          </p>
          ...
          <div className="border-app mt-3 flex flex-wrap gap-x-3 gap-y-1.5 border-t pt-3">
            {KETERANGAN.map((k) => (
              <span key={k.label} className="flex items-center gap-1.5">
                ...
                <span className="text-subtle text-[10px] font-semibold">{k.label}</span>
              </span>
            ))}
          </div>
        </div>
