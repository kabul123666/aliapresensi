...
export default async function HalamanProfil() {
  ...
  return (
    <div className="pb-6">
      <header className="bg-brand-700 pt-safe px-5 pb-6">
      ...
      {[
        ...
        <section key={blok.judul} className="mt-5 px-5">
          ...
          <dl className="border-app bg-surface mt-2 divide-y overflow-hidden rounded-[var(--radius-card)] border">
            {blok.isi.map((b) => (
              <div key={b.label} className="flex items-start gap-3 px-4 py-3">
                ...
                <dd className="text-body flex-1 text-[13px] font-semibold">{b.nilai}</dd>
              </div>
            ))}
          </dl>
        </section>
